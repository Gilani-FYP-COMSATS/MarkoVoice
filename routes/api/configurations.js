  ...
  async (req, res) => {
    ...
    try {
      ...
      if (configuration) {
        ...
        configuration = await ConfigurationModel.findOneAndUpdate(
          { user: req.user.id },
          { $set: inputFields },
          { new: true }
        );
...
router.get('/all', async (req, res) => {
  try {
    const configurations = await ConfigurationModel.find().populate('user', [
      'name',
      'email',
    ]);
    if (!configurations) {
...
router.get('/me', auth, async (req, res) => {
  try {
    const configurations = await ConfigurationModel.findOne({
      user: req.user.id,
...
router.delete('/:user_id', async (req, res) => {
  ...
  try {
    ...
    await ConfigurationModel.deleteOne({ user: uID });
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (configuration == null) {
        console.log('no configuration found for this user. First add config!');
        return res
          .status(404)
          .json('no configuration found for this user. First add config!');
      }
      let domainInfo = await ConfigurationModel.findOne({
        'domainInfo._id': req.params.dom_id,
      });
      if (domainInfo == null) {
        console.log('no domInfo found to update. First add domainInfo!');
        return res
          .status(404)
          .json('no domInfo found to update. First add domainInfo!');
      }
      configuration.domainInfo.unshift(newDomainInfo);
      await configuration.save();
      res.json(configuration);
